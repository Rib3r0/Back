...
let getCapitalEstado = (sigla) => {
   ...
   return capital != "" ? capital : false
}
// console.log(getCapitalEstado("MA"))

let getEstadosRegiao = (regiao) => {
   let listaEstados = []
   estadosCidades.estados.forEach( (estado) =>{
      if((estado.regiao).toUpperCase() == regiao.toUpperCase()){
         listaEstados.push({uf : estado.sigla, descricao: estado.nome})
      }
   })
   ...
   if(listaEstados.length == 0){
      ...
      return {regiao : regiao.toUpperCase(), estados: listaEstados }
}
}
